...
- **Fault scarps** — a separate line-tracing tool with the same Terra Draw plumbing and the same `pointerDistance` tightening; see *Fault scarps* below.
...
## Fault scarps — traced observations (public reads, editor-only writes)
...
- Client `inventory/static/inventory/js/scarps.js` + the panel in `home.html`, opened from **Analysis → Fault scarps (trace)** (editors) / **Fault scarps** (everyone else); clicking a line opens the card if it was closed. Draw → saves immediately and selects itself → write the note. Purple dashed lines with a wide transparent hit layer (a 2 px click target is not usable).
