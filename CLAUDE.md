...
**Archive modes** (`archive_mode` in the manifest): `warp` reprojects into the target zone (and applies `vertical_scale` for feet); `copy` is for a source that is already a COG in the target zone; `translate` is for a source in the target zone that is *not* a COG (plain/LZW, 128×128 blocks, external `.ovr`) — `gdal_translate -of COG` keeps the CRS, including a compound NAVD88 vertical, and the pixels bit-for-bit, where a warp onto its own grid would drop the vertical CRS. `src_srs` re-tags a file whose embedded CRS is broken without touching pixels: Sitka's is named UTM 8N but every Transverse Mercator parameter is zero, so GDAL put it at 7° E.
