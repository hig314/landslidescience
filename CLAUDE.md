...
- **Archive**: a COG in the correct local UTM zone (NAD83(2011)), on `/Volumes/Nunatak/lidar_build/cog/` (~38 GB, *not* in git and *not* on the droplet — destined for object storage). Readable over `/vsicurl`, so QGIS opens one straight off a URL; that is the point of hosting them, since the public sources are awkward to get data out of.
