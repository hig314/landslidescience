...
**Migrations (run on prod at deploy, idempotent, like `migrate_deprecated`):** `migrate_polygon_history`, `migrate_provisional_polygons`. (The standalone `/manage/new/` draw page was superseded by the main-map tool and is now unused — safe to retire.)
