...
## Fault scarps — traced observations (editor-only)
...
- **Editor-only including READS** (`inventory/scarps.py`, all views behind `inventory_editor_required`). These are working observations; a half-formed "possible scarp?" on the public map would read as a claim the project is not making. Making them public should be a deliberate act — a `published` column and a filter here — not a default.
...
- Client `inventory/static/inventory/js/scarps.js` + the panel in `home.html`, opened from **Analysis → Fault scarps (trace)**. Draw → saves immediately and selects itself → write the note. Purple dashed lines with a wide transparent hit layer (a 2 px click target is not usable).
