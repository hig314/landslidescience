...
**Archive modes** (`archive_mode` in the manifest): `warp` reprojects into the target zone (and applies `vertical_scale` for feet); `translate` is for a source already in the target zone — `gdal_translate -of COG` with `OVERVIEWS=IGNORE_EXISTING` keeps the CRS, including a compound NAVD88 vertical, and the pixels bit-for-bit, where a warp onto its own grid would drop the vertical CRS. Overviews are always recomputed from the pixels we ship (`OVERVIEWS=IGNORE_EXISTING`) so their provenance is known; `copy` mode exists but is unused. **A cautionary tale (2026-09-06):** an evening was spent "proving" that the Glen Alps, KBay and Seward vendor overviews were misregistered by 1–3 px with seams. They were not — checked with exact overview dimensions and grid-aligned windows, the vendor overviews are *closer* to a true block mean (0.18 m RMS) than GDAL's cascaded ones (0.35–0.55 m, sub-pixel drift on a non-divisible width). The "shifts" were the tests' own: reading an overview through an `out_shape` one column short makes GDAL resample it with a one-pixel jump at the middle column, and a comparison window whose origin is not a multiple of the overview factor is shifted by exactly `origin mod factor`. When comparing overviews to full-res: use `ceil(dim/factor)` dimensions, align windows to the factor, and demand RMS ≈ 0 before believing a shift. `src_srs` re-tags a file whose embedded CRS is broken without touching pixels: Sitka's is named UTM 8N but every Transverse Mercator parameter is zero, so GDAL put it at 7° E.
